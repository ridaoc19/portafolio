...
function Render({ handleOnClick, state, status }) {
  ...
  const information = [
    ...
    { type: "delete", color: "red", data: "Eleminaría el Cargo o Grado" },
    { type: "add", color: "green", data: "Al darle clic al el Cargo o Grado, abre la opción para agregar Funciones o Proyectos" }]
          ...
          return (
            <li key={e._id}>
              {!tooltip && <>
                ...
                </Tooltip><Tooltip text={`Eliminaria ${e.name}`} color={"red"} position="top">
                  <i id="position_delete" name="position_delete" onClick={handleOnClick} value={e._id}>{svg({ type: "delete", color: "red" })}</i>
                </Tooltip><Tooltip text={`Agrega Funciones o Proyectos de ${e.name}`} color={"green"} position="bottom">
